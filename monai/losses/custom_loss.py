...
class CustomLoss(nn.Module):
    ...
    """
    def __init__(self, to_onehot_y=False, weightAUF=2., weightBVTV=0.5, weight=0.5, delta=0.6, gamma=0.8, epsilon=1e-07):
        ...
        self.lossAUF = AsymmetricUnifiedFocalLoss(weight=self.weight, delta=self.delta, gamma=self.gamma)

    def forward(self, y_pred, y_true):
        # Obtain Asymmetric Unified Focal loss
        if self.to_onehot_y:
            y_true = one_hot(y_true, num_classes=y_pred.shape[1])
        y_pred = torch.softmax(y_pred, 1)  # IMPORTANT <-----------------------

        auf_loss = self.lossAUF(y_pred, y_true)
...
if __name__ == "__main__":
    ...
    for i in range(0, 96**3, 10000):
        ...
        x_lst.append(i)
        y_lst1.append(CustomLoss(to_onehot_y=True)(y_pred, y_true))
        y_lst2.append(DiceCELoss(to_onehot_y=True, softmax=True)(y_pred, y_true))
    plt.plot(x_lst, y_lst1, label="custom")
    plt.plot(x_lst, y_lst2, label="dice")
    plt.legend()
    plt.show()
