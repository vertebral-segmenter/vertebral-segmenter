...
Token = collections.namedtuple('Token', ['type', 'value', 'start', 'end', 'line'])
...
class Tokenizer:
    ...
    def get_tokens(self):
        for token_enum, token in enumerate(self._get_tokens()):
            ...
            if token_enum == 0:
                self.set_file_info(token)
            yield token

    def set_file_info(self, token):
        known_types = {
            '# HyperSurface 0.1 BINARY': {'type': 'HyperSurface', 'version': '0.1', 'is_binary': True},
            '# HyperSurface 0.1 ASCII': {'type': 'HyperSurface', 'version': '0.1', 'is_binary': False},
            '# AmiraMesh 3D BINARY 2.0': {'type': 'AmiraMesh', 'version': '2.0', 'is_binary': True},
            '# AmiraMesh 3D BINARY-LITTLE-ENDIAN 2.0': {'type': 'AmiraMesh', 'version': '2.0', 'is_binary': True},
            '# AmiraMesh 3D ASCII 2.0': {'type': 'AmiraMesh', 'version': '2.0', 'is_binary': False},
            '# AmiraMesh BINARY-LITTLE-ENDIAN 2.1': {'type': 'AmiraMesh', 'version': '2.1', 'is_binary': True},
        }
        if token[0] == TOKEN_COMMENT and token[1] in known_types:
            self.file_info = known_types[token[1]]
        else:
            warnings.warn('Unknown file type. Parsing may fail.')

    def process_bytedata_key(self, part, this_line, lineno, startcol, endcol):
    ...
    def process_token(self, part, this_line, lineno, startcol, endcol):
        if part in [b'{', b'}']:
            ...
            yield from self.process_bytedata_key(part, this_line, lineno, startcol, endcol)
        else:
        ...
        while len(self.buf) > 0:
            if (len(self.last_tokens) >= 3 and
                ...
                if self.file_info['type'] == 'HyperSurface':
                    if self.file_info.get('is_binary', BINARY_DEFAULT):
                        sizeof_element = 12  # 3x floats, 4 bytes per float
            ...
            if this_line.lstrip().startswith(b'#'):
                yield (
                    TOKEN_COMMENT, this_line[:-1].decode("utf-8"), (lineno, 0), (lineno, len(this_line) - 1), this_line)
                yield (
                    TOKEN_NEWLINE, this_line[-1:].decode("utf-8"), (lineno, len(this_line) - 1),
                    (lineno, len(this_line)),
                    this_line)
                ...
                for part_idx, part in enumerate(parts):
                    ...
                    yield from self.process_token(part, this_line, lineno, startcol, endcol)
