...
from data_preprocessing.image_analysis.nifti_processing import load_nifti, save_nifti, crop_roi
...
def main():
    scan_file = 'path/to/your/rat/vertebral/ct/scan.nii'
    label_file = 'path/to/your/l2/segmentation/label.nii'
    cropped_file = 'path/to/your/cropped/scan.nii'

    size = (32, 32, 32)  # Adjust this to the desired dimensions of the cropped region

    img, scan_data = load_nifti(scan_file)
    _, label_data = load_nifti(label_file)

    centroid = center_of_mass(label_data)
    cropped_data = crop_roi(scan_data, centroid, size)

    save_nifti(cropped_data, img, cropped_file)
